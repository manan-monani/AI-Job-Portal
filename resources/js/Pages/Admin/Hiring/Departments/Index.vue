...
import { Building2, Pencil, PlusCircle, Search, Trash2 } from 'lucide-vue-next';
import { PropType, ref, watch } from 'vue';

const props = defineProps({
    departments: Object as PropType<any>,
});
...
watch(
    search,
    debounce((value) => {
        router.get(admin.departments.index.url(), { search: value }, { preserveState: true, replace: true });
    }, 300),
);
...
const submit = () => {
    if (editingDepartment.value) {
        form.put(admin.departments.update.url(editingDepartment.value.id), {
            onSuccess: () => {
...
const toggleStatus = (department: any) => {
    router.patch(
        admin.departments.status.url(department.id),
        {
...
const handleDelete = () => {
    ...
    router.delete(admin.departments.destroy.url(departmentToDelete.value.id), {
        onSuccess: () => {
        ...
        <div class="animate-fade-in space-y-6">
            ...
            <div class="flex flex-col items-start justify-between gap-6 text-start md:flex-row md:items-center">
                <div class="text-start">
                    <h2 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">{{ __('Departments') }}</h2>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Manage company departments and their status.') }}</p>
                </div>
                <button
                ...
                >
                    <PlusCircle :size="18" class="me-2" /> {{ __('Add New Department') }}
                </button>
            </div>
            ...
            <div class="overflow-hidden rounded-[2rem] border border-slate-200/60 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <table class="w-full border-collapse text-start">
                        <thead>
                            <tr class="border-b border-slate-100 bg-slate-50/50 dark:border-slate-700/50 dark:bg-slate-800/20">
                                <th class="px-6 py-4 text-start text-[10px] font-bold tracking-widest text-slate-400 uppercase">{{ __('Name') }}</th>
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-sm font-bold text-slate-900 dark:text-white">{{ department.name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="line-clamp-1 max-w-xs text-xs text-slate-500">{{ department.description || '---' }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <label class="relative inline-flex cursor-pointer items-center">
                                        <input
                                            ...
                                            class="peer sr-only"
                                            @change="toggleStatus(department)"
                                        />
                                        <div
                ...
                <!-- Pagination -->
                <div v-if="departments.links.length > 3" class="flex justify-end border-t border-slate-100 p-6 dark:border-slate-700">
                    <div class="flex space-x-1">
                        <Link
                            v-for="(link, key) in departments.links"
                            :key="key"
                            :href="link.url || '#'"
                            v-html="link.label"
            ...
            <!-- Create/Edit Modal -->
            <Modal :show="showModal" :title="editingDepartment ? __('Edit Department') : __('Add New Department')" @close="showModal = false">
                <form id="department-form" @submit.prevent="submit" class="space-y-4">
                    <div>
                        <label class="mb-1 block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Department Name') }}</label>
                        <input
                            v-model="form.name"
                            type="text"
                ...
                <template #footer>
                    <div class="flex justify-end gap-3">
                        ...
                        >
                            ...
                            ></span>
                            {{ editingDepartment ? __('Update') : __('Create') }}
                        </button>
                    </div>
                </template>
            </Modal>
            ...
            <ConfirmationModal
                ...
                :message="__('Are you sure you want to delete this department? This action cannot be undone.')"
