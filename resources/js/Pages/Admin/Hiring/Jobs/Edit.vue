...
import { AlertCircle, ArrowLeft, Briefcase, Clock, Info, Save, Tag } from 'lucide-vue-next';
import { PropType } from 'vue';

const props = defineProps({
    job: Object as PropType<any>,
...
const form = useForm({
    ...
    salary_period: props.job.salary_period || 'monthly',
    ...
    tags: props.job.tags?.map((t: any) => t.id) || [],
});

const submit = () => {
    form.put(admin.jobs.update.url(props.job.id));
};
...
<template>
    <Head :title="__('Edit Job Opening')" />
        ...
        <div class="animate-fade-in mx-auto max-w-5xl space-y-6">
            ...
            <div class="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
                <div>
                    ...
                    <h2 class="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">{{ __('Edit Job Opening') }}</h2>
                    <div class="mt-1 flex items-center gap-4">
                        <p class="text-sm text-slate-500 italic dark:text-slate-400">{{ props.job.title }}</p>
                        <div class="flex items-center gap-1.5 text-[10px] text-slate-400">
                            <Clock :size="12" />
                            <span>{{ __('Updated') }}: {{ new Date(props.job.updated_at).toLocaleDateString() }}</span>
                        </div>
                    </div>
                </div>
                <button
                ...
                >
                    <Save :size="18" class="me-2" /> {{ __('Update Job Post') }}
                </button>
            </div>

            <form @submit.prevent="submit" class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <!-- Main Form (Same as Create.vue) -->
                <div class="space-y-6 text-start lg:col-span-2">
                    <!-- General Information -->
                ...
                <div class="space-y-6 text-start">
                    ...
                    <div class="space-y-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                        ...
                        <div v-if="form.job_type !== 'remote'" class="animate-slide-down">
                            <label class="mb-1.5 block text-sm font-bold text-slate-700 dark:text-slate-300"
                    ...
                    <div class="space-y-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                        ...
                        <div class="custom-scrollbar flex max-h-[200px] flex-wrap gap-2 overflow-y-auto pr-2">
                            <button
                                v-for="tag in tags"
                                :key="tag.id"
                                type="button"
